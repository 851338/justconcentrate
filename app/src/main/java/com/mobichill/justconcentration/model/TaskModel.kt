...
fun TaskModel.toFireStoreMap(): MutableMap<String, Any?> {
    return mutableMapOf(
...
fun TaskModel.fromFireStoreMap(docId: String, map: Map<String, Any?>): TaskModel? {
    try {
        ...
        val task = TaskModel(
            ...
            requestCode = (map["requestCode"] as? Long)?.toInt() ?: 0, // FireStore stores numbers as Long by default
            completed = map["completed"] as? Boolean ?: false,
            alarmSoundUri = map["alarmSoundUri"] as? String ?: "",
            createdAt = map["createdAt"] as? Long ?: System.currentTimeMillis(), // Default if missing
